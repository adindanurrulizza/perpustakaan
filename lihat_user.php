...
<hr>
<p><a href="media.php?module=tambah_user" class='btn btn-info'>Tambah Data</a><br>
...
<td width='21%' align='center' class='td style3'>
 <a  href='?module=user&act=delete&id=<?echo$r[username];?>' onclick="return confirm('Anda yakin untuk menghapus data ini ?')">
 <button class='btn btn-danger'>Delete</button></a>  <a  href='?module=edit_user&id=<?echo$r[username];?>'><button class='btn btn-warning'>Edit</button></a></td>
</tr>
<?
$no++;
}
...
echo "Halaman : ";
		...
		if($halaman > 1){
			$previous = $halaman - 1;
			echo "<a href='$_SERVER[PHP_SELF]?halaman=1&module=user' class=\"btn btn-primary edit\">&laquo; First</a>  
			     <a href='$_SERVER[PHP_SELF]?halaman=$previous&module=user' class=\"btn btn-primary edit\">&lsaquo; Prev</a>";
		}else{
		...
		for($i=$halaman-2; $i<$halaman; $i++){
			if($i < 1)
				continue;
			$angka .= "<a href='$_SERVER[PHP_SELF]?halaman=$i&module=user' class=\"btn btn-primary edit\">$i</a> ";
		}
		...
		$angka .= ($halaman+2 < $jmlhal ? " ... <a href='$_SERVER[PHP_SELF]?halaman=$jmlhal&module=user' class=\"btn btn-primary edit\">$jmlhal</a> " : " ");
		...
		if($halaman< $jmlhal){
			$next = $halaman + 1;
			echo "<a href='$_SERVER[PHP_SELF]?halaman=$next&module=user' class=\"btn btn-primary edit\">Next &rsaquo;</a>  
			     <a href='$_SERVER[PHP_SELF]?halaman=$jmlhal&module=user' class=\"btn btn-primary edit\">Last &raquo;</a>";
		}else{
...
<?
if($_GET[module]=='user' and $_GET[act]=='delete'){
