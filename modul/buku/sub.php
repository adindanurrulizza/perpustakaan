...
case "tampil";
$query=mysql_query("select * from subkategori");
echo"<h4>Data Sub Kategori</h4>";
echo"<input type=button value='Tambah Data Sub Kategori' onclick=location.href='?module=sub&aksi=tambahbuku' class='btn btn-primary'></br></br>";
echo"<center><div style='overflow:auto;height:400px;width:950;padding:0px;margin-bottom:0px'>  
<table id='tabel' class='table table-bordered table-striped'>
<tr  align='center'>
<td width='75'>Kode Sub Kategori</td>
<td width='75'>Kode  Kategori</td>
<td width='75'>Nama Sub Kategori</td>
...
echo"<td>$tampil[id_sub_kategori]</td>";
echo"<td>$tampil[id_ketegori]</td>";
echo"<td>$tampil[nm_sub]</td>";



echo"<td align='center'><a onclick=\"return confirm('Anda Yakin Menghapus Data Ini?')\" href='?module=sub&aksi=hapus&id=$tampil[id_sub_kategori]'> Hapus</a></td> ";
...
?>
<center><table id='tabeledit'><form action='?module=sub&aksi=input' name='postform' method=POST>
<tr><td>ID Sub Kategori </td><td>
<input size='35' type='text' name='t1' maxlength='20'></td></tr>
<tr><td>ID Kategori  </td><td>
<select  name='t2' maxlength='20'>
...
<tr><td>Nama Sub Kategori </td><td>
<input size='35' type='text' name='t3' maxlength='60'></td></tr>
...
// AKSI HAPUS
case "hapus":
mysql_query("DELETE FROM subkategori WHERE  id_sub_kategori='$_GET[id]'");
	echo '<script>alert(\'Data Berhasil Dihapus\')
	setTimeout(\'location.href="?module=sub&aksi=tampil"\' ,0);</script>';
break;
...
if ((empty($_POST['t3'])) )
{
echo"<p>Nama Sub Kategori Wajib Diisi<input type='button' onclick=self.history.back() value='back'/>";
}
Else
{		


 $qry	= mysql_query("SELECT MAX(CONCAT(LPAD((RIGHT((no_induk_buku),4)+1),4,'0')))FROM subkategori");
$qry2	= mysql_query("SELECT MIN(CONCAT(LPAD((RIGHT((no_induk_buku),4)),4,'0')))FROM subkategori");	
...
$kodeauto = $kode[0];
 ...
 $sql = mysql_query("INSERT INTO  subkategori VALUES('$_POST[t1]',
 '$_POST[t2]',
 '$_POST[t3]')"); 
