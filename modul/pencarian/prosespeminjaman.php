...
$result=mysql_query("select * from peminjaman,anggota,buku where (peminjaman.no_agt = anggota.no_agt) and (peminjaman.kode_buku = buku.no_induk_buku) and (peminjaman.buku like '%$key%' or buku.no_induk_buku like '%$key%' or status like '%$key%' or call_number_1 like '%$key%') order by no_peminjaman DESC",$con); 
...
<td width='10%' colspan='3'>No Call</td>
...
echo"<td>$tampil[call_number_1]</td><td>$tampil[call_number_2]</td><td>$tampil[call_number_3]</td>";
