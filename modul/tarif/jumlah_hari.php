...
case "tampil";
$query=mysql_query("select * from jumlah_hari");
echo"<h4>Jumlah Hari Maksimal Pinjam</h4>";
...
echo"<td align='center'><span style='font-size:20pt;'>$tampil[jumlah] Hari</span></td>";
echo"<td align='center'><a href=?module=jumlah_hari&aksi=edit_jumlah&id=$tampil[id_jumlah] class='btn btn-primary'>Ubah</td>";
$baris++;}
echo"</tr>";
...
case "edit_jumlah":
echo"<h4>Edit Jumlah Hari Maksimal Pinjam</h4>";
$db="select * from jumlah_hari where id_jumlah='1'";
$qri=mysql_query($db);
$row=mysql_fetch_array($qri);
echo"<form action='?module=jumlah_hari&aksi=update&id_jumlah=$row[id_jumlah]' method=POST>";
...
// AKSI UPDATE jumlah
case "update":
mysql_query("UPDATE jumlah_hari SET jumlah='$_POST[jumlah]'		
			where id_jumlah='$_GET[id_jumlah]'");
echo '<script>alert(\'Data Berhasil Diedit\')
	setTimeout(\'location.href="?module=jumlah_hari&aksi=tampil"\' ,0);</script>';
break;
}
}
