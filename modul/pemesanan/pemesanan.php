...
echo"<td width='3%'><a onclick=\"return confirm('Anda Yakin Menghapus Data Ini?')\" href='?module=pemesanan&aksi=hapus&id=$tampil[no_pemesanan]'>Hapus</td>";
$no++;
$baris++;}
echo"</tr>";
echo"</table></center>";
...
<form action='?module=pemesanan&aksi=input' name='postform'
		method='POST'
...
mysql_query("DELETE FROM pemesanan WHERE no_pemesanan='$_GET[id]'");
	...
	setTimeout(\'location.href="?module=pemesanan&aksi=tampil"\' ,0);</script>';
	}
break;
...
echo "<script>
	...
	echo '<script>setTimeout(\'location.href="?module=pemesanan&aksi=tampil"\' ,0);</script>';
}
else{
